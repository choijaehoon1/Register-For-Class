/**
 *<pre>
 * Class Name : AdminLectureAddController
 * Description : 
 ...
 *    |_ AdminLectureAddController.java
 * 
 * </pre>
 * @date : 2019. 12. 13.
 * @version : 
 * @author : 신찬미
 */
public class AdminLectureAddController implements Initializable {
	@FXML private Button btnMake;//등록
	@FXML private Button btnDel;//취소
	@FXML private TextField txtlectureCode;
	@FXML private TextField txtlecturName;
	@FXML private TextField txtprofessorCode;
	...
	/**
	 * 강의 개설 데이터 입력
	 * 입력 양식 검사,데이터 유효성 검사 event (강의코드,교수,강의실)
	 * Table view 추가 event
	 * @author : 신찬미
	 * @param event
	 */
	public void btnMakeAction(ActionEvent event) {
		...
		if(true == dao.isLectureCodeExists(lectureCode)) {
			alert.setContentText("중복된 강의코드입니다.");
			alert.showAndWait();
			txtlectureCode.requestFocus();
			return;
		}
		
		if(true == dao.isLectureroomExistsAdd(lectureRoom,lectureDay,lectureStartTime,lectureEndTime)) {
			alert.setContentText("사용중인 강의실 입니다.");
			alert.showAndWait();
			txtlectureRoom.requestFocus();
			return;
		}
		if(true == dao.isProfessorCodeExists(professorCode,lectureDay,lectureStartTime,lectureEndTime)) {
			alert.setContentText("수업중인 교수 입니다.");
			alert.showAndWait();
			txtprofessorCode.requestFocus();
			return;
		}
		...
		alert.setContentText("등록 하시겠습니까?");
		...
		if(flag == 1) {
			alert.setContentText("강의가 등록 되었습니다.");
			alert.showAndWait();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		txtlectureStartTime.setValue("09:00");
		...
		txtlectureEndTime.setValue("10:00");
	}
